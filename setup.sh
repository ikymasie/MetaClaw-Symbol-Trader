...
if [ "$SKIP_BACKEND_ENV" = false ]; then
    echo ""
    echo -e "  ${CYAN}${BOLD}  ── Alpaca (Paper Trading) ──${RESET}"
    echo -e "  ${DIM}  Get keys at: https://alpaca.markets → Paper Trading → API Keys${RESET}"
    prompt_with_default ALPACA_KEY  "  Alpaca API Key" ""
    prompt_with_default ALPACA_SECRET "  Alpaca Secret Key" "" true
    prompt_with_default ALPACA_URL "  Alpaca Base URL" "https://paper-api.alpaca.markets"
...
if [ -f "backend/.env" ]; then
    if grep -q "ALPACA_API_KEY=.\+" backend/.env 2>/dev/null; then
        echo -e "  ${CHECK}  Alpaca API         ${GREEN}configured${RESET}"
    else
        echo -e "  ${WARN}  Alpaca API         ${YELLOW}not configured${RESET}"
    fi
