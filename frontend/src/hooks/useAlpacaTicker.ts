...
/**
 * useAlpacaTicker
 * Connects to /ws/ticker/{symbol} and streams live Alpaca bar + bot context.
 ...
 */
export function useAlpacaTicker(symbol: string | null, enabled: boolean = true): TickerState {
  const [bars, setBars] = useState<AlpacaBar[]>([]);
  const [lastQuote, setLastQuote] = useState<AlpacaQuote | null>(null);
